...
bool GraphicsSystem::Frame()
{
	...
	if (mInputClass->IsMouseRightButtuonPressed() && fps >= 5 && fps <= 1000000)
	{
		...
		if (rotateY <= 90.0f&&rotateY >= -90.0f)
		{
			...
			GCamera->Pitch((float)mouseYOffset*deltaTime*0.1f);
		}
	
		//进行视角左右的旋转
		GCamera->RotateY((float)mouseXOffset*deltaTime*0.1f);
	}
...
void GraphicsSystem::RenderDebugWindow()
{
	...
	GShaderManager->uiShader->SetTexture("ShaderTexture", mSSRBuffer->GetGBufferSRV(SSRBufferType::VIEW_NORMAL));
	GShaderManager->uiShader->Apply();
	mDebugWindow->Render(10, 480);

	GShaderManager->uiShader->SetTexture("ShaderTexture", mSSRBuffer->GetGBufferSRV(SSRBufferType::VIEW_POS));
	GShaderManager->uiShader->Apply();
