...
////NEW!!!!///////
double HitSphere(const vec3& center, double radius, const ray& inRay) {
//////////////////
    ...
    if (discriminant < 0) 
        ...
        return (-b - sqrt(discriminant)) / (2.0 * a);//assume the closest hit point(smallest t)
    //////////////////
}
...
color RayColor(const ray& inRay) {
    ...
    double t = HitSphere(sphere_center, 0.5, inRay);
    if (t > 0.0) {
        vec3 N = unit_vector(inRay.At(t) - sphere_center);
        return 0.5 * color(N.x() + 1, N.y() + 1, N.z() + 1);//convert from (-1,1) to [0,1] 
    }
    //////////////////

    vec3 unit_direction = unit_vector(inRay.GetDirection()); // -1 < unit_direction < 1
    t = 0.5 * (unit_direction.y() + 1.0); // 0 <= t <= 1.0
